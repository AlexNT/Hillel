...
class Rectangle:
    ...
    def __add__(self, other):
        # Сумуємо площі трикутників
        new_square = self.get_square() + other.get_square()
        # Вибираємо одну сторону нового прямокутника як середнє геометричне
    ...
    def __mul__(self, n):
        ...
        # Створюємо новий прямокутник з площею, що дорівнює площі базового прямокутника, помноженій на n
        new_width = math.sqrt(new_square)
        new_height = new_square / new_width
        return Rectangle(new_width, new_height)
